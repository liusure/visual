...
var toBytes = function (s) { return toArray(s).map(function (c) { return c.charCodeAt(0); }); };
...
var getFileType = function (input) {
    ...
    if (check([0x50, 0x4b, 0x3, 0x4])) {
        ...
        if (check(oxmlContentTypes, {
            ...
            var nextZipHeaderIndex = function (arr) {
                return findIndex(arr, function (el, i, arr) {
                    return arr[i] === 0x50 &&
                        arr[i + 1] === 0x4b &&
                        arr[i + 2] === 0x3 &&
                        arr[i + 3] === 0x4;
    ...
    if (check([0x1a, 0x45, 0xdf, 0xa3])) {
        var sliced_1 = buf.subarray(4, 4 + 4096);
        var idPos = findIndex(sliced_1, function (el, i, arr) { return arr[i] === 0x42 && arr[i + 1] === 0x82; });
        if (idPos !== -1) {
            var docTypePos_1 = idPos + 3;
            var findDocType = function (type) {
                return toArray(type).every(function (c, i) { return sliced_1[docTypePos_1 + i] === c.charCodeAt(0); });
            };
