    ...
    visitor: {
      ...
      ImportDeclaration(path, state) {
        ...
        if (t.isStringLiteral(node.source, {
          ...
          const specifierCount = specifiers.length; // no import 'zent';
...
function buildImportReplacement(specifier, types, state, originalPath) {
  ...
  const options = state.opts,
        data = state.data;
  const libName = getLibraryName(state);
  const noModuleRewrite = options.noModuleRewrite;

  if (data.MODULE_MAPPING.hasOwnProperty(importedName)) {
    const automaticStyleImport = options.automaticStyleImport,
          useRawStyle = options.useRawStyle;
    const rule = data.MODULE_MAPPING[importedName]; // js

    if (!noModuleRewrite) {
      replacement.push(types.importDeclaration(buildImportSpecifier(types, rule.isDefaultExport, importedName, localName), types.stringLiteral(getJavaScriptPath(rule.js, libName))));
    } // style


    if (automaticStyleImport) {
      if (!rule.style) {
        throw originalPath.buildCodeFrameError('Please upgrade zent to >= zent@7.0.0');
      }

      rule.style.forEach(path => {
        if (data.STYLE_IMPORT_MAPPING[path] === undefined) {
          replacement.push(types.importDeclaration([], types.stringLiteral(getStylePath(path, libName, !!useRawStyle))));
...
function buildImportSpecifier(types, isDefaultExport, importedName, localName) {
  ...
  return [types.importSpecifier(types.identifier(localName), types.identifier(importedName))];
}
